...
- **`POST /api/v1/webhooks/stripe`** (inbound, added 2026-09-24): Stripe events, accepted only with a valid `Stripe-Signature` (`STRIPE_WEBHOOK_SECRET`). `payment_intent.succeeded` captures, `payment_intent.canceled` voids — both idempotent on `payments_gateway_reference_uq`, so redelivery records nothing twice. Other events are acknowledged and ignored.
