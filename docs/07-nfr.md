...
| 11 | **Laravel 11 carries unpatched framework advisories (recorded 2026-09-25).** `composer audit` against `laravel/framework` v11.56.1 reports three advisories covering two distinct flaws. **CRLF injection in the default `email` rule** (GHSA-5vg9-5847-vvmq, high; also listed as CVE-2026-48019) is fixed in 12.60.0 and never backported to 11. **Temporary signed URL path confusion** (GHSA-crmm-hgp2-wgrp, medium) is fixed in 12.61.1. Two mitigations are in place: (a) every email field refuses CR and LF before the `email` rule runs (`AuthFields::email()`, 05.13 §20), and (b) email verification uses an HMAC checked in the controller (`EmailVerificationLink`), not a Laravel temporary signed route. **Both are compensating controls, not fixes.** They hold only while every new email field goes through `AuthFields::email()` and no code uses `URL::temporarySignedRoute`/`signed` middleware. §6.3's signed-URL file serving must not be built on Laravel's signed routes while on 11. They also leave §6.3's "build fails on high severity" `composer audit` gate failing permanently. **Resolution: upgrade to Laravel 12 (≥ 12.61.1, which clears both flaws).** This is a stack amendment: CLAUDE.md names Laravel 11 and must be amended first, in its own commit. Tracked in ROADMAP §0.8 | **Yes, before go-live** |
